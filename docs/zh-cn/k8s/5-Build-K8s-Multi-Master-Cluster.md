...
- [Kubernates下载](https://kubernetes.io/zh-cn/releases/download/)
- [在 Linux 系统中安装并设置 kubectl](https://kubernetes.io/zh-cn/docs/tasks/tools/install-kubectl-linux/)

- [开源项目Helm 用于安装Dashbaord](https://github.com/helm/helm/releases)
...
```shell
sudo dnf install -y vim wget tar unzip ebtables ethtool nc net-tools yum-utils iproute-tc ipset ipvsadm 
...
#### 4.3.1 下载安装包
- [Docker安装包下载](https://download.docker.com/linux/centos/8/x86_64/stable/Packages/)
```shell
mkdir -p ~/docker
...
curl -sSLf https://download.docker.com/linux/centos/8/x86_64/stable/Packages/docker-scan-plugin-0.9.0-3.el8.x86_64.rpm -o ~/docker/docker-scan-plugin-0.9.0-3.el8.x86_64.rpm
...
```shell
# 安装离线包
sudo dnf install ~/docker/containerd.io-1.6.9-3.1.el8.x86_64.rpm
sudo dnf install ~/docker/docker-buildx-plugin-0.14.0-1.el8.x86_64.rpm
sudo dnf install ~/docker/docker-ce-26.1.3-1.el8.x86_64.rpm
sudo dnf install ~/docker/docker-ce-cli-26.1.3-1.el8.x86_64.rpm
sudo dnf install ~/docker/docker-ce-rootless-extras-26.1.3-1.el8.x86_64.rpm
sudo dnf install ~/docker/docker-compose-plugin-2.6.0-3.el8.x86_64.rpm
sudo dnf install ~/docker/docker-scan-plugin-0.9.0-3.el8.x86_64.rpm
...
```shell
mkdir -p ~/k8s/
curl -sSLf https://github.com/Mirantis/cri-dockerd/releases/download/v0.3.14/cri-dockerd-0.3.14-3.el8.x86_64.rpm -o ~/k8s/cri-dockerd-0.3.14-3.el8.x86_64.rpm

sudo rpm -ivh ~/k8s/cri-dockerd-0.3.14-3.el8.x86_64.rpm
...
curl -sSLf  https://github.com/Mirantis/cri-dockerd/releases/download/v0.3.16/cri-dockerd-0.3.16.amd64.tgz -o ~/k8s/cri-dockerd-0.3.16.amd64.tgz

scp ~/k8s/cri-dockerd-0.3.16.amd64.tgz diginn@192.168.137.121:/home/diginn/k8s/
scp ~/k8s/cri-dockerd-0.3.16.amd64.tgz diginn@192.168.137.123:/home/diginn/k8s/
scp ~/k8s/cri-dockerd-0.3.16.amd64.tgz diginn@192.168.137.131:/home/diginn/k8s/
scp ~/k8s/cri-dockerd-0.3.16.amd64.tgz diginn@192.168.137.132:/home/diginn/k8s/
scp ~/k8s/cri-dockerd-0.3.16.amd64.tgz diginn@192.168.137.133:/home/diginn/k8s/

# 解压
tar -zxvf ~/k8s/cri-dockerd-0.3.16.amd64.tgz -C ~/
...
### 5.3 安装 kubelet、kubeadm 和 kubectl
...
#### 6.3.1 `192.168.137.121`作为VIP的主节点，配置文件如下
```shell
...
#### 6.3.2 `192.168.137.122`作为VIP的备份节点，配置文件如下
```shell
cat <<EOF | sudo tee /etc/keepalived/keepalived.conf
! Configuration File for keepalived
global_defs {
...
#### 6.3.3 `192.168.137.123`作为VIP的备份节点，配置文件如下
```shell
cat <<EOF | sudo tee /etc/keepalived/keepalived.conf
! Configuration File for keepalived
global_defs {
...
mkdir ~/k8s/
# 下载calico.yaml
# wget https://raw.githubusercontent.com/projectcalico/calico/refs/heads/release-v3.25/manifests/calico.yaml -0 ~/k8s/calico.yaml
curl -sSLf https://docs.tigera.io/archive/v3.25/manifests/calico.yaml  -o ~/k8s/calico.yaml

# 修改配置文件中的 CALICO_IPV4POOL_CIDR
sudo vim ~/k8s/calico.yaml
...
```shell
# 下载脚本
wget https://raw.githubusercontent.com/yuyicai/update-kube-cert/refs/heads/master/update-kubeadm-cert.sh  -O ~/k8s/update-kubeadm-cert.sh
...
```shell
# Helm 下载
wget https://get.helm.sh/helm-v3.17.2-linux-amd64.tar.gz -O ~/k8s/helm-v3.17.2-linux-amd64.tar.gz

# 解压
tar -zxvf ~/k8s/helm-v3.17.2-linux-amd64.tar.gz
...
# 查看版本
helm version
[diginn@k8s-master-01 k8s]$ helm version
...
wget https://github.com/kubernetes/dashboard/releases/download/kubernetes-dashboard-7.7.0/kubernetes-dashboard-7.7.0.tgz -O kubernetes-dashboard-7.7.0.tgz
...
```shell
tar -zxvf kubernetes-dashboard-7.7.0.tgz
cd kubernetes-dashboard
