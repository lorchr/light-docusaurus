- [SpringBoot整合Canal+RabbitMQ监听数据变更](https://mp.weixin.qq.com/s/HBsksd2NQMQUmw5wfGZeRw)
...
`/home/admin/canal-server/conf/example/instance.properties`
  - 这里的`/example`是指instance即实例名，要和上面`canal.properties`内instance配置对应，canal会为实例创建对应的文件夹，一个Client对应一个实例
...
- 新建`exchange`，命名为：`canal-exchange`
- 新建`queue`，命名为：`canal-queue`
- 绑定`exchange`和`queue`，`routing-key`设置为：`canal-routing-key`，这里对应上面`instance.properties`的`canal.mq.topic`
