- [Redis最佳实践：7个维度+43条使用规范，带你彻底玩转Redis | 附实践清单](https://mp.weixin.qq.com/s/H6GFmzPq0Tl_j9NghcEt-w)
...
- `List`类型：执行多次 `LPOP/RPOP`，直到所有元素都删除完成
- `Hash/Set/ZSet`类型：先执行 `HSCAN/SSCAN/SCAN` 查询元素，再执行 `HDEL/SREM/ZREM` 依次删除每个元素
